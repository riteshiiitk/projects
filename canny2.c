...
test 
        canny_edge archbloco.1.pgm 1 0.3 0.8

--------------F609DE99117C596D65E63491
Content-Type: text/x-csrc; charset=us-ascii;
 name="canny_edge.c"
Content-Transfer-Encoding: 7bit
Content-Disposition: inline;
 filename="canny_edge.c"
         ...
         float tlow, float thigh, unsigned char **edge, char *fname)
{
   ...
   ****************************************************************************/
   ...
   /*   
   {
     int i,max = 0;
     unsigned char *mag;
     mag = (unsigned char *) calloc(rows*cols,sizeof(unsigned char));
     for (i=0;i <rows*cols;i++)
       {
         if (magnitude[i] > max) max = magnitude[i];
         if (magnitude[i] > 255) mag[i] = 255;
         else mag[i] = magnitude[i];
       }

     printf("Max :%d\n",max);
     write_pgm_image("grad.pgm",mag, rows, cols, "", 255);
   ...
   */
   ...
   /*write_pgm_image("nms.pgm",nms, rows, cols, "", 255);*/
